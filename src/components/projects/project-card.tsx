...
const ProjectCard = component$(({ project }: { project: Project }) => {
    return (
        <div class={"mx-2 md:mx-auto md:w-2/3"}>
            <main class="rounded-xl bg-primary px-4 py-6 sm:p-6 md:px-8 md:py-10">
                <div class="mx-auto grid max-w-4xl grid-cols-1 lg:max-w-5xl lg:grid-cols-2 lg:gap-x-20">
                    <div class="relative col-start-1 row-start-1 flex flex-col-reverse rounded-lg bg-gradient-to-t from-black/75 via-black/0 p-3 sm:row-start-2 sm:bg-none sm:p-0 lg:row-start-1">
                        <Link
                            ...
                            target="_self"
                        >
                            <h1 class="mr-auto mt-1 block text-lg font-semibold text-secondary hover:underline  md:text-2xl">
                                {project.name}
                            </h1>
                        </Link>
                        <p class="text-sm font-medium leading-4 text-secondary hover:underline">
                            {project.contributors?.map((contributor, index) => (
                                <>
                                    ...
                                    {index <
                                    (project.contributors?.length ?? 0) - 1
                                        ? ", "
                                        : ""}
                                </>
                            ))}
                    ...
                    <div class="row-start-2 mt-4 flex flex-col justify-center space-y-2 text-xs font-medium sm:row-start-3 sm:mt-1 md:mt-2.5 lg:row-start-2">
                        {project.progress && (
                            <>
                                ...
                                <div class="m-0 text-gray-300  sm:w-2/3">
                                    <span>
                                        {project.progress * 100}% complete
                                    </span>
                                    <div class="mt-1 h-1.5 w-full rounded-full bg-gray-500">
                                        <div
                                            class="h-1.5 rounded-full bg-white "
                                            style={`width: ${
                    ...
                    <p class="col-start-1 mt-4 text-sm leading-6 text-secondary  sm:col-span-2 lg:col-span-1 lg:row-start-4 lg:mt-6">
                        {project.description}
                    </p>
                </div>
            </main>
