...
export function TimeBetweenSettings({
  ...
  return createPortal(
    <div data-modal-root="rest-settings">
      <div
        aria-hidden="true"
        className="fixed inset-0 z-40 bg-black/30 sm:bg-black/20"
        onClick={onCancel}
      />
      <div className="pointer-events-none fixed inset-0 z-50 flex items-end sm:items-start sm:justify-end sm:p-6">
        <section
          ref={dialogRef}
        ...
        >
          <div className="mb-3 flex items-center justify-between">
            ...
            <button
              ...
              className="min-h-11 min-w-11 rounded-full p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            >
              <svg
                className="h-5 w-5"
                fill="none"
          ...
          >
            This duration applies after each repetition and stretch. Changes
            take effect only when you apply them.
          </p>
