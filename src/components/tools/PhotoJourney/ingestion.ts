...
export function acceptFiles(
  files: readonly File[],
  existing: readonly JourneyPhoto[],
) {
  const accepted: File[] = [];
  const tracks: File[] = [];
...
function bundleEntryName(path: string) {
  ...
  if (/-original\.gpx$/i.test(base))
    return base.replace(/-original\.gpx$/i, ".gpx");
  return base;
}
...
function isIgnorableEntry(path: string) {
  ...
  return (
    ...
    base === "Thumbs.db" ||
    base.startsWith("._") ||
    /^readme\.txt$/i.test(base) ||
    /\.(json|geojson)$/i.test(base)
  );
}
...
function selectJourneyGpx(paths: readonly string[]) {
  ...
  const rootJourney = paths.find(
    (path) => /^journey\.gpx$/i.test(path) && !path.includes("/"),
  );
  if (rootJourney && paths.some((path) => /^(days|recordings)\//i.test(path)))
    return new Set([rootJourney]);
 ...
 */
export async function expandJourneyArchives(
  ...
  let projectManifest: PhotoJourneyProjectManifest | undefined;
  ...
  for (const [archiveIndex, archive] of archives.entries()) {
    ...
    const manifestEntry = entries.find(
      (entry) => entry.name === PROJECT_MANIFEST_FILENAME,
    );
    if (manifestEntry && !projectManifest) {
      try {
        projectManifest = parseProjectManifest(
        ...
        if (!projectManifest)
          skipped.push({
            file: archive,
            reason:
              "project settings are invalid or use an unsupported version",
          });
        else bundleTitle ??= projectManifest.title;
      } catch {
        skipped.push({
          file: archive,
          reason: "project settings could not be read",
        });
      }
    }
    const candidates = entries.filter((entry) => !isIgnorableEntry(entry.name));
    const gpxPaths = candidates
      .filter((entry) => /\.gpx$/i.test(entry.name))
    ...
    if (selected.length > MAX_ARCHIVE_ENTRIES) {
      skipped.push({
        file: archive,
        reason: `ZIP exceeds the ${MAX_ARCHIVE_ENTRIES} importable file limit`,
      });
      continue;
    }
    let usable = 0;
    ...
    for (const [index, entry] of selected.entries()) {
      ...
      try {
        ...
        if (uncompressedBytes > MAX_ARCHIVE_UNCOMPRESSED_BYTES) {
          skipped.push({
            file: archive,
            reason: `ZIP contents exceed the ${Math.round(MAX_ARCHIVE_UNCOMPRESSED_BYTES / 1024 / 1024)} MB limit`,
          });
          break;
        }
        usable += 1;
        files.push(
          new File([bytes as BlobPart], name, {
            type: type || undefined,
            lastModified: entry.date?.valueOf() ?? archive.lastModified,
          }),
        );
      } catch {
        skipped.push({
          file: new File([], name),
          reason: `could not be read from ${archive.name}`,
        });
      }
    ...
    if (!usable && !skipped.some((entry) => entry.file === archive)) {
      skipped.push({
        file: archive,
        reason: "no photos or GPX found in this ZIP",
      });
      continue;
    }
    ...
    if (!bundleTitle) {
      const manifest = entries.find(
        (entry) => !entry.dir && /(^|\/)journey\.json$/i.test(entry.name),
      );
      if (manifest) {
        try {
          const parsed = JSON.parse(await manifest.async("string")) as {
            title?: unknown;
          };
          if (typeof parsed.title === "string" && parsed.title.trim())
            bundleTitle = parsed.title.trim().slice(0, 120);
        } catch {
          /* A hand-made zip keeps its files even without a readable manifest. */
        }
      }
    }
  }

  return { expanded: { files, bundleTitle, projectManifest }, skipped };
}
