...
export function formatDayKeyRange(startKey?: string, endKey?: string) {
  ...
  const end =
    endKey && /^\d{4}-\d{2}-\d{2}$/.test(endKey)
...
export function journeySummary(
  ...
  const dates = photos
    .flatMap((photo, index) => {
      ...
      if (placement?.instant !== undefined)
        return [new Date(placement.instant)];
      return photo.metadata.capturedAt ? [photo.metadata.capturedAt] : [];
    })
    .sort((a, b) => +a - +b);
  const altitudes = photos.flatMap((photo) =>
    photo.metadata.altitude === undefined ? [] : [photo.metadata.altitude],
  );
  ...
  const allDates = dates
    .concat(track?.start ? [track.start] : [], track?.end ? [track.end] : [])
    .sort((a, b) => +a - +b);
  return {
    ...
    distanceKm: track
      ...
      : located.reduce(
          (total, point, index) =>
            total + (index ? distanceKm(located[index - 1], point) : 0),
          0,
        ),
    track,
    ...
    tripDays: orderedDateKeys.length
      ? Math.round(
          (Date.parse(`${orderedDateKeys.at(-1)}T00:00:00Z`) -
            Date.parse(`${orderedDateKeys[0]}T00:00:00Z`)) /
            86_400_000,
        ) + 1
      : 0,
...
export function exportJourney(
  ...
  const placed = photos
    .map((photo, index) => ({
      photo,
      ...exportPlacement(photo, placements?.[index]),
    }))
  ...
  const metadata = photos.map((photo, index) => {
    ...
    const {
      ...
      capturedAtWallClock: _capturedAtWallClock,
    ...
    return {
      ...
      cameraFixOffByM:
        placement.discrepancyM === undefined
          ? undefined
          : Math.round(placement.discrepancyM),
      ...
      content: JSON.stringify(
        {
          ...
          summary: journeySummary(
            photos,
            placements,
            track ? trackStats(track) : undefined,
            timezone,
            track,
          ),
          photos: metadata,
  ...
  const photoFeatures: Array<{
    ...
    properties: {
      ...
      place:
        entry.source === "track" &&
        (entry.discrepancyM === undefined || entry.discrepancyM > 60)
          ? "recorded position"
    ...
    content: JSON.stringify(
      {
        ...
        features: [...recordedFeatures, ...photoFeatures],
      },
      null,
      2,
    ),
...
function readme(photos: readonly JourneyPhoto[], options: BundleOptions) {
  ...
  const unassigned = photos.filter(
    (photo, index) =>
      !exportPlacement(photo, options.placements?.[index]).coordinates,
  ).length;
  const unresolvedTime = photos.filter(
    (photo, index) =>
      photo.metadata.capturedAtWallClock &&
      exportPlacement(photo, options.placements?.[index]).time === undefined,
  ).length;
  return [
    options.title,
    "",
    ...
    options.track
      ? `Route from your GPX: ${options.track.points.length} recorded points.`
      : "Distance estimated from photo positions.",
    corrected
      ? `${corrected} photo${corrected === 1 ? "" : "s"} placed by timecode on the track rather than by the camera's own fix.`
      : "",
    `${unassigned} photo${unassigned === 1 ? "" : "s"} omitted from GPX waypoints because no position was assigned.`,
    unresolvedTime
      ? `${unresolvedTime} camera time${unresolvedTime === 1 ? "" : "s"} remain unresolved and are omitted from GPX timestamps.`
      : "",
    "",
    ...
    options.includePhotos
      ? "photos/          the original files, unmodified"
      : "",
    "",
...
export type ScopedBundleOptions = {
  ...
  track?: Track;
  includePhotos?: boolean;
  /** Present only for a restorable project ZIP. Ordinary exports omit editor state. */
  projectManifest?: PhotoJourneyProjectManifest;
};
...
export async function buildScopedBundle(options: ScopedBundleOptions) {
  const { default: JSZip } = await import("jszip");
  const zip = new JSZip();
  const includedRecordings = options.recordings.filter(
    (recording) => recording.included,
  );
  // A saved project must keep excluded recordings too, otherwise reopening it cannot restore
  // their include state or let the user turn them back on. Ordinary exports remain scoped to
  // the included route sources.
  const recordingsToArchive = options.projectManifest
    ? options.recordings
    : includedRecordings;
  const scopeTrack =
    options.track ??
    (includedRecordings.length
      ? mergeTracks(includedRecordings.map((recording) => recording.track))
      : undefined);
  const days = deriveJourneyDays(
    options.photos,
    options.placements,
    options.recordings,
    options.timezone,
  );
  const allResult = exportJourney(
    options.photos,
    "gpx",
    options.title,
    options.placements,
  ...
  if (options.projectManifest)
    zip.file(
      PROJECT_MANIFEST_FILENAME,
      serializeProjectManifest(options.projectManifest),
    );

  const usedDayNames = new Set<string>();
  for (const [index, day] of days.entries()) {
    const entries = scopedPhotos(
      options.photos,
      options.placements,
      day.key as DayScope,
      options.timezone,
    ...
    zip.file(
      `days/${String(index + 1).padStart(2, "0")}-${name}.gpx`,
      exportJourney(
        dayPhotos,
        "gpx",
        `${options.title} · ${day.label}`,
        dayPlacements,
        dayTrack,
        options.timezone,
      ).content,
  ...
  for (const [index, recording] of [...recordingsToArchive]
    .sort((a, b) => a.importOrder - b.importOrder)
    .entries()) {
    const base = safeFilename(
      recording.name || recording.file.name.replace(/\.gpx$/i, ""),
      `recording-${index + 1}`,
    );
    let name = base;
    let suffix = 2;
    ...
    const sourceEntries = options.photos.flatMap((photo, photoIndex) => {
      ...
      return placement?.recordingId === recording.id &&
    ...
    zip.file(
      `recordings/${name}-original.gpx`,
      await recording.file.arrayBuffer(),
    );
  }
  ...
  const unresolvedTime = options.photos.filter(
    (photo, index) =>
      photo.metadata.capturedAtWallClock &&
      options.placements[index]?.instant === undefined,
  ).length;
  const conflicts = options.placements.filter(
    (placement) => placement.conflict,
  ).length;
  zip.file(
    "readme.txt",
    [
      ...
      unresolvedTime
        ? `${unresolvedTime} camera time${unresolvedTime === 1 ? "" : "s"} remain unresolved; generated GPX omits those timestamps.`
        : "",
      conflicts
        ? `${conflicts} photo${conflicts === 1 ? "" : "s"} still need a location choice.`
        : "",
      "",
      ...
      options.includePhotos
        ? "photos/ contains the original photo files, unmodified."
        : "",
      options.projectManifest
        ? "photo-journey-project.json restores the editor state when this ZIP is opened in Photo Journey."
        : "",
    ]
      .filter(Boolean)
      .join("\n"),
