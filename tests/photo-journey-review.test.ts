...
import {
  reviewCounts,
  reviewItems,
} from "../src/components/tools/PhotoJourney/review";
...
function photo(id: string): JourneyPhoto {
  ...
  return {
    ...
    thumbnailUrl: id,
    importOrder: 0,
    metadata: normalizeMetadata(file, {}, 1, 1),
  };
}
...
describe("photo review queue", () => {
  ...
  test("keeps a camera GPS difference informational because the GPX stays authoritative", () => {
    ...
    expect(reviewItems(photos, placements, {})[0].conflict).toBe(false);
  });
  ...
  test("overlapping or unavailable recordings remain flagged despite a saved choice", () => {
    ...
    expect(
      reviewCounts(reviewItems(photos, placements, { a: "track", b: "track" }))
        .conflicts,
    ).toBe(2);
  });

  test("keeps GPS differences attached to stable photo IDs when capture sorting changes", () => {
    const a = photo("a"),
      b = photo("b");
    const placements: Placement[] = [
    ...
    expect(
      items.filter((item) => item.difference).map((item) => item.photo.id),
    ).toEqual(["b", "a"]);
  });
  ...
  test("a photo with GPS but no capture time still needs time review", () => {
    ...
    expect(reviewCounts(reviewItems([a], placements, {}))).toMatchObject({
      time: 1,
      unlocated: 0,
      "needs-review": 1,
    });
