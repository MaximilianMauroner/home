...
import {
  ...
  journeyTravelZoom,
  smoothProgress,
} from "../src/components/tools/PhotoJourney/motion";
import {
  buildTimeline,
  BURST_HOLD_DURATION,
...
const photos = Array.from(
  ...
  (_, index): JourneyPhoto => ({
    ...
    importOrder: index,
    metadata: {
      capturedAt: new Date(2025, 0, 1, 12, 0, index),
...
const motionAt = (elapsed: number, reduced = false) =>
  journeyMotion(timelineAt(elapsed, timeline), reduced);

describe("Photo Journey motion", () => {
  test("reveals the photo quickly and keeps it through departure", () => {
    expect(motionAt(stop.revealStart)).toMatchObject({
      drawer: 1,
      image: 0,
      photoTransition: 0,
      checkpoint: 0,
  ...
  test("keeps route motion linear throughout the approach", () => {
  ...
  test("samples the same state after seeking backward and holds photos still", () => {
    ...
    expect(motionAt(stop.revealEnd + 100)).toMatchObject({
      drawer: 1,
      image: 1,
      checkpoint: 1,
    });
    expect(motionAt(stop.departureStart - 100)).toMatchObject({
      drawer: 1,
      image: 1,
      checkpoint: 1,
    });
  ...
  test("crossfades only within a burst transition, including backward seeks", () => {
    ...
    const sample = (elapsed: number) =>
      burstPhotoProgress(timelineAt(elapsed, timeline), 1, false);
  ...
  test("reduced motion shows settled content immediately and no drawer on cards", () => {
    expect(motionAt(stop.revealStart, true)).toMatchObject({
      drawer: 1,
      image: 1,
      checkpoint: 1,
    });
